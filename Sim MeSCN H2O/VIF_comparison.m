...
%% CLS method example (linear absorbance, propagating time constant error)
	...
	J_p = ex2_J(:,[1,2,4,6,7]);
	J_p_J_tau = -J_p'*J_tau;
	J_aug = [J_p;transpose(J_p_J_tau)];
	N_d = size(J_p,1);
	LA_noise = (1e-5)*ones(1,N_d);
	sigma_tau = [p.kubo1_t.val,p.kubo2_t.val]*0.01;
	
	cov_aug = inv(J_p'*J_p)*J_aug'*diag([LA_noise.^2,sigma_tau.^2])*J_aug*inv(J_p'*J_p);
	var_lin_ind = (LA_noise(1)).^2./diag(J_p'*J_p);
