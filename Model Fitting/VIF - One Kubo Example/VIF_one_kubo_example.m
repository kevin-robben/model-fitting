...
%% linear absorption example
	...
	w3_fit_range = [2162.4-10,2162.4+10]; % upper 95% of linear absorption
	[n3_min,n3_max] = nearest_index(x.w3,w3_fit_range);
	fxn = @(x,p) Lin_Abs(x,p,p.A01.val,p.c.val,[p.kubo1_t.val],[p.kubo1_D2.val],p.T_hom_inv.val,[n3_min,n3_max],x.w3);
	p_LA = p;
	...
	%% plot VIF
		...
		set(ax_1,'ytick',[1e0,1e1,1e2,1e3,1e4,1e5],'yticklabels',{'1','10^1','10^2','10^3','10^4','10^5'},'TickLength',[0.02,0])
		xlim(ax_1,[0.5,LA_aux.num_var+0.5])
		ylim(ax_1,[1,1e5])
		ylabel(ax_1,'VIF')
		title(ax_1,{'Naive Fit to','Linear Absorbance'});
%% CLS method example (linear absorbance)
	w3_fit_range = [2162.4-5.5,2162.4+5.5]; % upper 80% of linear absorption
	[n3_min,n3_max] = nearest_index(x.w3,w3_fit_range);
	fxn = @(x,p) Lin_Abs(x,p,p.A01.val,p.c.val,[p.kubo1_t.val],[p.kubo1_D2.val],p.T_hom_inv.val,[n3_min,n3_max],x.w3);
	p_CLS = p_LA;
	p_CLS.kubo1_t.var = 0;
	...
	%% plot linear absorbance
		...
		ylim(ax,[0,1])
		xlabel('frequency');ylabel('Absorbance (arb. unit)');
		title('Linear Asorbance Spectrum for CLS')
	%% plot VIF
		...
		set(ax_2,'ytick',[1e0,1e1,1e2,1e3,1e4,1e5],'yticklabels',{'1','10^1','10^2','10^3','10^4','10^5'},'TickLength',[0.02,0])
...
%% model fitting example
	%% plot VIF
		...
		ylim(ax_3,[1,1e5])
		set(ax_3,'ytick',[1e0,1e1,1e2,1e3,1e4,1e5],'yticklabels',{'1','10^1','10^2','10^3','10^4','10^5'},'TickLength',[0.02,0])
