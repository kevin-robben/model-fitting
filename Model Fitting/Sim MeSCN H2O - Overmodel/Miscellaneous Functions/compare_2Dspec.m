function compare_2Dspec(fig,x,v1_plot_lim,v3_plot_lim,T,Data_spec,Model_spec)
	...
	data_ax = nexttile(layout,1);
		plot_2Dspec(data_ax,x,v1_plot_lim,v3_plot_lim,Data_spec(:,:,n_Tw),sprintf('Experimental Data at T_w = %.3gps',x.Tw(n_Tw)))
		data_clim = caxis(data_ax);
	model_ax = nexttile(layout,2);
		plot_2Dspec(model_ax,x,v1_plot_lim,v3_plot_lim,Model_spec(:,:,n_Tw),sprintf('Model Fit at T_w = %.3gps',x.Tw(n_Tw)))
		model_clim = caxis(model_ax);
	res_ax = nexttile(layout,3);
		Diff = Data_spec-Model_spec;
		plot_2Dspec(res_ax,x,v1_plot_lim,v3_plot_lim,Diff(:,:,n_Tw),sprintf('Residual (Data - Model) at T_w = %.3gps',x.Tw(n_Tw)))
