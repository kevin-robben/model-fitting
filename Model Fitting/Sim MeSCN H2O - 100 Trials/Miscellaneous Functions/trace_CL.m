function [CL_v3, v1_axis, CLS] = trace_CL(v1,v1_range,v3,v3_range,spec,peak_model)
    %% gather indicies of min and max fitting range of along pump and probe axes
        [n1_min,n1_max] = nearest_index(v1,v1_range);
        [n3_min,n3_max] = nearest_index(v3,v3_range);
    %% trim down axes and spectra to fitting range
        v1_axis = v1(n1_min:n1_max);
        v3_axis = v3(n3_min:n3_max);
        spec1 = spec(n1_min:n1_max,n3_min:n3_max);
        CL_v3 = zeros(size(v1_axis)); %initialize center line array
    ...
    %% for loop over each probe slice
        for i=1:numel(v1_axis)
            [amp_guess,indx_guess] = min(spec1(i,:));
            switch peak_model
                case 'Asymmetric Lorentzian'
                    %% define initial guess and fit peak
                        fit_options = fitoptions(fit_options,'StartPoint',[amp_guess,v3_axis(indx_guess),6,0]);
                        [f1,gof,output] = fit(v3_axis',spec1(i,:)',fit_type,fit_options);
                    %% numerical peak search
                        x = v3_axis(1):1e-3:v3_axis(numel(v3_axis));
    ...
    %% calculate CLS from center line
        CLS = sum((v1_axis-mean(v1_axis)).*(CL_v3-mean(CL_v3)))/sum((v1_axis-mean(v1_axis)).^2);
end
