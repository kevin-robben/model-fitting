...
%% initialize figures
	initial_fit_fig = figure;
		set(initial_fit_fig,'Position',[300 200 600 200]);
	...
	CLS_fig = figure;
		set(CLS_fig,'Position',[50 50 300 250]);
		CLS_layout = tiledlayout(CLS_fig,1,1,'Padding','compact');
	LA_fig = figure;
		set(LA_fig,'Position',[50 50 300 250]);
		LA_layout = tiledlayout(LA_fig,1,1,'Padding','compact');
	trial_params_fig = figure;
		set(trial_params_fig,'Position',[50 50 600 200]);
		trial_params_layout = tiledlayout(trial_params_fig,1,3,'Padding','compact','TileSpacing','compact');
		annotation(trial_params_fig,'textbox',[0.01 0.9 0.025 0.085],'String','(A)','LineStyle','none','FitBoxToText','off');
		annotation(trial_params_fig,'textbox',[0.31 0.9 0.025 0.085],'String','(B)','LineStyle','none','FitBoxToText','off');
		annotation(trial_params_fig,'textbox',[0.63 0.9 0.025 0.085],'String','(C)','LineStyle','none','FitBoxToText','off');
...
for trial=1:100
		...
		%% update fitting report
			...
			%% add reticles to fitting report to symbolize true values
				%% homogeneous reticle
					...
					line(ax,[0.2857,0.2857],[0.02857-0.01,0.02857+0.01],'Color','k','LineStyle','--');
					line(ax,[0.2857-0.08,0.2857+0.08],[0.02857,0.02857],'Color','k','LineStyle','--');
				%% frequency reticle
					ax = update_fig.Children.Children(5);
					line(ax,[2162.4,2162.4],[27-0.6,27+0.6],'Color','k','LineStyle','--');
					line(ax,[2162.4-0.2,2162.4+0.2],[27,27],'Color','k','LineStyle','--');
				%% Kubo reticles
					ax = update_fig.Children.Children(3);
					line(ax,[0.4,0.4],[33.64-3,33.64+3],'Color','k','LineStyle','--');
					line(ax,[0.4-0.4,0.4+0.4],[33.64,33.64],'Color','k','LineStyle','--');
					line(ax,[1.7-0.4,1.7+0.4],[6.76,6.76],'Color','k','LineStyle','--');
					line(ax,[1.7,1.7],[6.76-3,6.76+3],'Color','k','LineStyle','--');
	...
	%% update plot of parameters obtained from last trial relative to true parameters
		...
		ax = nexttile(trial_params_layout,2);
            ...
            xlim(ax,[0.5,5.5]);ylim(ax,[0.9 1.1]);
			xticks(ax,[1 2 3 4 5])
            xticklabels(ax,{'\Delta^2_1','\Delta^2_2','\tau_1','\tau_2','T_{hom}^{-1}'})
			set(ax,'Box','on','TickLength',[0.02,0]);
	...
	%% update plot of average parameters (w/ 95% C.I.) obtained from all trials relative to true parameters
		if trial>2
			...
			xlim(ax,[0.5,5.5]);ylim(ax,[0.95 1.05]);
			xticks(ax,[1 2 3 4 5])
			xticklabels(ax,{'\Delta^2_1','\Delta^2_2','\tau_1','\tau_2','T_{hom}^{-1}'})
			set(ax,'Box','on','TickLength',[0.02,0]);
