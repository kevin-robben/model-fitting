function [apo_mask,inv_apo_mask] = apo_masks(x)
	%% define regular apo mask
		...
		apo_w3 = reshape(ones(size(x.w3)),[1,x.N3,1]);
		apo_start = 2.0;
		apo_end = 3.95;
		for i=1:x.N1
			if x.t1(i) >= apo_start
	...
	%% define inverse apo mask
		...
		for i=1:numel(apo_w3)
			if abs(apo_w3(i)) < (1e-5)
				inv_apo_w3(i) = 0;
			else
				inv_apo_w3(i) = 1/apo_w3(i);
			end
		end
		inv_apo_mask = inv_apo_t1.*inv_apo_Tw.*inv_apo_w3;
end