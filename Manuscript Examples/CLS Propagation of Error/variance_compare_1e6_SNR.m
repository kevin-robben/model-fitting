...
%% initialize figure
	...
	comp_fig = openfig('Templates\composite figure template.fig');
%% load p
...
%% initialize axes and true values
	...
	noise = (1e-6)*randn(numel(true_lin_abs),100);
	y_lim = [-2e-6,2e-6];
...
%% run empirical variance trial set for CLS method with fixed tau
	...
	annotation(comp_fig,'textbox',[0.09,0.73,0.3,0.0348],'String',str,'linestyle','none','FontSize',9)
...
%% run empirical variance trial set for CLS method with uncertain tau
	...
	str = sprintf('%i%%    %i%%   %i%%  %i%% %i%%     -     -',(sum(check,1)));
	annotation(comp_fig,'textbox',[0.09,0.51,0.3,0.0348],'String',str,'linestyle','none','FontSize',9)
...
%% run empirical variance trial set for linear absorption
	...
	str = sprintf('%i%% %i%% %i%% %i%% %i%%',(sum(check,1)));
	annotation(comp_fig,'textbox',[0.075,0.29,0.32,0.0348],'String',str,'linestyle','none','FontSize',8)
...
%% plot CLS to LA std ratio
	...
	legend(ax2,'Abs. Certain CLS \tau','Uncertain CLS \tau')
